...
```bash
cd ~/projects/hapax-council && LITELLM_API_KEY=$(hapax-secret litellm/master-key) LANGFUSE_PUBLIC_KEY=$(hapax-secret langfuse/public-key) LANGFUSE_SECRET_KEY=$(hapax-secret langfuse/secret-key) uv run python -m agents.briefing --hours 24 --save
```
