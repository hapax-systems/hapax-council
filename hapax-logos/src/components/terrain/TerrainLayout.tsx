...
function StudioParamSync() {
  ...
  useEffect(() => {
    ...
    // Activate backend preset via API if specified in URL
    if (presetName) {
      fetch(`/api/studio/presets/${encodeURIComponent(presetName)}/activate`, { method: "POST" }).catch(() => {});
    }
    if (source) {
...
function StudioKeyboardHandler() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      // E: toggle HLS mode, auto-advance to core
