...
pub struct DynamicPipeline {
    ...
    /// Temporal textures are undefined immediately after GPU allocation.
    /// A temporal shader must never sample that undefined memory; doing so
    /// can turn a restart or plan change into a full-frame white/noise pane.
    temporal_textures_primed: HashSet<String>,
...
impl DynamicPipeline {
    pub fn new(
        ...
        let mut pipeline = Self {
            ...
            temporal_textures_primed: HashSet::new(),
            uniform_buffer,
            shm_output,
    ...
    pub fn try_reload(&mut self, device: &wgpu::Device) -> bool {
        ...
        let required_temporal_names = active_passes
            .iter()
            .filter(|(_target, pass)| pass.inputs.iter().any(|n| n.starts_with("@accum_")))
            .map(|(_target, pass)| pass.node_id.clone())
            .collect::<HashSet<_>>();
    ...
    pub fn resize(&mut self, device: &wgpu::Device, width: u32, height: u32) {
        ...
        self.temporal_textures_primed.clear();
    ...
    fn ensure_temporal_texture(&mut self, device: &wgpu::Device, node_id: &str) {
        ...
        self.temporal_textures_primed.remove(node_id);
    }

    fn prime_temporal_textures(&mut self, encoder: &mut wgpu::CommandEncoder) {
        let unprimed = self
            .temporal_textures
            .keys()
            .filter(|name| !self.temporal_textures_primed.contains(*name))
            .cloned()
            .collect::<Vec<_>>();

        for name in unprimed {
            if let Some(tex) = self.temporal_textures.get(&name) {
                let _pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("prime temporal accumulator"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view: &tex.view,
                        resolve_target: None,
                        ops: wgpu::Operations {
            ...
            self.temporal_textures_primed.insert(name);
        }
    }
...
"#;
    ...
    #[test]
    fn autonomous_drift_shader_inventory_parses_with_shared_uniforms() {
        let shader_root =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../agents/shaders/nodes");
        let passes = crate::effect_drift::SHADERS
            .iter()
            .map(|shader| {
                (
                    "main".to_string(),
                    make_pass(shader.name, shader.shader_file),
                )
            })
            .collect::<Vec<_>>();
        ...
        assert!(
            failures.is_empty(),
            "every autonomous drift shader must parse before runtime recruitment: {:?}",
            failures
        );
    }
