...
- **earlyoom** (`/etc/default/earlyoom`, source: `config/earlyoom/default`): fires SIGTERM @ 5% avail / 5% swap-free, SIGKILL @ 2.5%.
  ...
  - `--avoid`: `Hyprland|pipewire|wireplumber|dockerd|containerd|bluetoothd|systemd|foot|waybar|hapax-logos|hapax-imaginati|hapax-daimonion|studio-composit|logos-api|officium-api` (best-effort process-comm matching; long native process names use `/proc/<pid>/comm` 15-byte truncation. Python-hosted services are protected by OOMScoreAdjust drop-ins plus the root enforcer, not by service-name matching here.)
...
- **P0 OOM containment package** (`scripts/install-p0-oom-containment`): installs source-controlled recovery policy into `/etc/default/earlyoom`, `/etc/systemd/system/*.service.d/oom-protect.conf`, `/etc/systemd/system/system.slice.d/oom-containment.conf`, `/etc/systemd/system/user-1000.slice.d/oom-containment.conf`, `/etc/systemd/system/user@1000.service.d/oom.conf`, `~/.config/systemd/user/app.slice.d/oom-containment.conf`, and the root-side `hapax-oom-score-enforce` service/timer plus the activation-independent `hapax-root-failure-intake@.service` bridge. It also installs `hapax-oom-policy-audit` and `hapax-root-required-deploy-audit` under `/usr/local/sbin` and copies their user services/timers into `~/.config/systemd/user`, so recurring safety audits do not depend on a mutable source-activation alias. Root invocation resolves UID 1000's account/home and runs user-manager operations through `runuser` with `XDG_RUNTIME_DIR=/run/user/1000`; override `HAPAX_OOM_TARGET_UID`, `HAPAX_OOM_TARGET_USER`, and `HAPAX_OOM_TARGET_HOME` together for a different operator account.
  - `user-1000.slice` and `user@1000.service`: `MemoryHigh=80G`, `MemoryMax=96G`, `MemorySwapMax=8G`, `MemoryLow=16G`, `MemoryMin=8G`; `user@1000.service` also carries `OOMScoreAdjust=100`, restoring the packaged kill ordering so the whole user manager does not shield every interactive workload. The UID-level cap covers both `app.slice` and sibling session scopes, so an app runaway plus a long-lived interactive session cannot recreate the host-wide memory collapse.
  ...
  - Broadcast-critical user services: pipewire/pipewire-pulse/wireplumber (`-900`), hapax-daimonion (`-500`), studio-compositor (`-800`), and hapax-imagination (`-800`) carry source-controlled user drop-ins as desired policy. Because the user manager is deliberately killable (`OOMScoreAdjust=100`), an unprivileged user service cannot lower live child scores after boot; `hapax-oom-score-enforce.timer` runs as root on a 30-second interval with `AccuracySec=1s` and writes `/proc/<pid>/oom_score_adj` for the user manager and protected user-service cgroup PIDs only when the live value drifted.
  ...
  - Recheck: `scripts/install-p0-oom-containment --check`, `scripts/install-p0-oom-containment --verify-live`, `/usr/local/sbin/hapax-oom-policy-audit --json`, `systemctl is-enabled hapax-oom-score-enforce.timer earlyoom.service`, `systemctl is-active hapax-oom-score-enforce.timer earlyoom.service`, and `systemctl show hapax-oom-score-enforce.service -p Result -p ExecMainStartTimestamp`. Also require `systemctl --user is-enabled hapax-oom-policy-audit.timer hapax-root-required-deploy-audit.timer` and `systemctl --user is-active hapax-oom-policy-audit.timer hapax-root-required-deploy-audit.timer`. Inspect recurrence state with `jq . ~/.cache/hapax/p0-incident-intake/state.json` and `tail -n 20 ~/.cache/hapax/p0-incident-intake/events.jsonl`.
  - Root-required post-merge installers return rc `77` when non-interactive sudo is unavailable; `hapax-post-merge-deploy` atomically stages disposable packages at `~/.cache/hapax/post-merge-root-required/<sha>/`, while the installed source snapshots, per-package SHA receipts, and shared lock live durably under `~/.local/state/hapax/root-required/`. Every install, including a direct repair from a clean worktree, takes the exclusive lock and verifies every owned source file against the claimed commit before live mutation; the recurring audit takes the shared lock and binds each installed snapshot back to its receipt commit without a source-activation fallback. Run each staged RUNBOOK command after `sudo -v`; a successful installer records its owned source files plus its SHA receipt and drains only the exact validated `<sha>/<package>` source named by `HAPAX_ROOT_REQUIRED_DRAIN_DIR`. A deferred ancestor of a newer installed receipt is drained as superseded without live mutation; a missing receipt/snapshot or divergent, modified, or unverifiable package fails closed. Post-merge deployment never republishes installed source after the owning installer releases its lock and never bulk-deletes deferrals from other SHAs.
...
- **UPS power-alert provenance** (`scripts/install-apcupsd-power-alerts`): installs the apcupsd config/hooks/helper into `/etc/apcupsd`, hard-codes production hooks to `/etc/apcupsd/hapax-power-event.py`, installs `/etc/logrotate.d/hapax-ups-power-events` for `/var/log/hapax/ups-power-events.jsonl`, and enables/starts `apcupsd.service` so the sole shutdown-policy owner survives reboot; live verification requires both enabled and active state. The logrotate stanza uses `su root root` because `/var/log/hapax` is group-writable for local audit writers. Battery-transfer receipts say only that the transfer event itself does not request shutdown (`event_requests_shutdown=false`), while global `shutdown_requested` remains unknown; restoration receipts are also neutral because either event can follow a real `doshutdown`. The dedicated `doshutdown` hook attempts to record both fields as true and warns on logging or notification failure, but always returns `0` so telemetry failure cannot suppress apccontrol's default safety action; a blocked helper may therefore complete without a receipt. apcaccess and ntfy each have a one-second internal ceiling, and the entire helper runs under a hard three-second `timeout`, including blocked audit-file I/O.
  - `apcupsd` is the sole UPS shutdown-policy owner. `/etc/UPower/UPower.conf.d/90-hapax-apcupsd-owner.conf` keeps UPower available for status/history while pinning its critical action to `Ignore`; apcupsd retains the configured 20% charge / 5-minute runtime shutdown thresholds.
  - Recheck: `scripts/install-apcupsd-power-alerts --check` and `scripts/install-apcupsd-power-alerts --verify-live`; use `scripts/install-apcupsd-power-alerts --install --verify-live` to repair live drift. Root invocation still resolves UID 1000's home for the shared durable state; override `HAPAX_APCUPSD_TARGET_UID` and `HAPAX_APCUPSD_TARGET_HOME` together for another operator account.
