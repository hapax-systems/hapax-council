...
- **P0 OOM containment package** (`scripts/install-p0-oom-containment`): installs source-controlled recovery policy into `/etc/default/earlyoom`, `/etc/systemd/system/*.service.d/oom-protect.conf`, `/etc/systemd/system/system.slice.d/oom-containment.conf`, `/etc/systemd/system/user.slice.d/oom-containment.conf`, `/etc/systemd/system/user-1000.slice.d/oom-containment.conf`, `/etc/systemd/system/user@1000.service.d/oom.conf`, `~/.config/systemd/user/{app,session}.slice.d/oom-containment.conf`, and the root-side `hapax-oom-score-enforce` service/timer plus the activation-independent `hapax-root-failure-intake@.service` bridge. It also owns and installs `hapax-local-judge.service`, the exact host-profile table, selected zram policy, `hapax-oom-policy-audit`, and `hapax-root-required-deploy-audit`; ordinary post-merge deployment therefore stages a changed judge unit with the root-required package instead of copying or restarting it through the generic user-unit path. Recurring safety audits do not depend on a mutable source-activation alias. Both audit oneshots have a two-minute start deadline so a blocked package lock, user bus, UPS query, sudo probe, or Git read cannot suppress later timer recurrence indefinitely. The package is bound to account `hapax`, UID 1000, canonical host identities, and fixed unit names; test overrides are refused for production install destinations.
  ...
  - Docker containment: every existing `hapax-github-mcp-*` container is converged to `512M` RAM / `768M` total memory+swap, and `hapax-local-judge` to `4G` / `6G`. The launch sources carry the same caps, no target uses `--oom-kill-disable`, and both installer and recurring audit inspect running, stopped, and `Created` containers by stable full container ID. A failed update or inspect may be treated as an ephemeral disappearance only after an unfiltered successful enumeration proves that both the exact ID and the governed container name are absent; renaming the original ID or replacing it under the same governed name therefore remains a hard failure.
