...
- **P0 OOM containment package** (`scripts/install-p0-oom-containment`): installs source-controlled recovery policy into `/etc/default/earlyoom`, `/etc/systemd/system/*.service.d/oom-protect.conf`, `/etc/systemd/system/system.slice.d/oom-containment.conf`, `/etc/systemd/system/user.slice.d/oom-containment.conf`, `/etc/systemd/system/user-1000.slice.d/oom-containment.conf`, `/etc/systemd/system/user@1000.service.d/oom.conf`, `~/.config/systemd/user/{app,session}.slice.d/oom-containment.conf`, and the root-side `hapax-oom-score-enforce` service/timer plus the activation-independent `hapax-root-failure-intake@.service` bridge. It also owns and installs `hapax-local-judge.service`, the exact host-profile table, selected zram policy, `hapax-oom-policy-audit`, and `hapax-root-required-deploy-audit`; ordinary post-merge deployment therefore stages a changed judge unit with the root-required package, and `systemd/scripts/install-units.sh` skips it, so neither generic path copies, symlinks, enables, or restarts the unit. Recurring safety audits do not depend on a mutable source-activation alias. Both audit oneshots have a two-minute start deadline so a blocked package lock, user bus, UPS query, sudo probe, or Git read cannot suppress later timer recurrence indefinitely. The package is bound to account `hapax`, UID 1000, exact canonical host identities without suffix normalization, and fixed unit names; test overrides are refused for production install destinations.
  ...
  - Root-required post-merge installers return rc `77` when non-interactive sudo is unavailable; `hapax-post-merge-deploy` derives each package trigger set from the versioned ownership manifests on both sides of the deployed diff and atomically stages reconstructible packages at `~/.cache/hapax/post-merge-root-required/<sha>/`, while desired SHAs, installed source snapshots, installed SHA receipts, and the shared lock live durably under `~/.local/state/hapax/root-required/`. The complete scripts refuse whole-script root execution: run them as UID 1000 after `sudo -v`, so the shared lock is opened only by its owner and only narrow `run_root` commands cross the privilege boundary. `/usr/bin/python3` wrappers in post-merge deploy, both installers, and the recurring root-required audit open the lock with `O_NOFOLLOW`, validate and lock the opened inode, and pass only that descriptor to their child; each child revalidates the descriptor against the current regular path inode and acquires the appropriate exclusive/shared lock itself, so a pre-seeded environment marker or unrelated inherited descriptor cannot bypass serialization. The recurring OOM policy audit independently opens and validates the same inode and holds a shared lock across its complete live readback, so first activation or a timer tick waits for any exclusive APC/OOM install instead of reporting transient package state. When both packages changed, the apcupsd package runs first because the OOM verifier requires the recovery daemon to be enabled and active. The desired SHA is recorded before each install attempt, so loss of cached metadata remains audit-failing until that desired package is installed. Desired transitions are monotonic under the shared lock: post-merge and both installers preserve a newer desired descendant, accept a newer candidate, and accept an ancestry-divergent squash/rebase transition only when both versioned ownership manifests are identical and every owned package file is byte-identical. Every production install, including a governed same-SHA repair, is accepted only through the exact-release-bound sealed-source path; a bare installer `--install` and the retired caller-supplied finalizer protocol both refuse before lock or live mutation. At installer exec, the helper supplies an exact closed inherited environment; before any external command, each installer uses Bash builtins to verify the raw `/proc/$$/environ` name set and fixed command-resolution values. The scripts refuse sourcing and Bash invocation without privileged mode, bind Bash-generated `PWD`/`SHLVL`, and introduce no additional exported names; Bash may still synthesize `_` for child commands. The helper and installer independently execute the exact packaged `hapax-post-merge-deploy` authority verifier against the canonical activation release and exact effect-descriptor scopes. The installer repeats that check before its first live effect and before receipts or deferral state advance; no caller-supplied executable is an authentication input. Authenticated `--install` always performs live verification before installed-source snapshots or receipts advance. Unauthenticated installation is available only to tests that bind every writable destination and mutating executable beneath one secure temporary root. A manifest may add owned paths, but removing or renaming an installed path fails before mutation until explicit governed live-removal handling exists. Installer verification and the recurring audits require every root artifact to be root-owned with its exact mode beneath a root-owned, non-writable parent; the root-required audit also binds each installed snapshot back to its receipt commit without a source-activation fallback. Installers default commit lookup to the stable source-activation worktree rather than a developer worktree. Each deferral keeps `RUNBOOK.txt` as non-executable metadata and emits a live, fully quoted `env -i` command that resolves and validates the account's canonical source-activation worktree before streaming the desired SHA's helper blob into isolated Python; caller-selected repository paths are not an authentication input. That helper holds every manifest blob in a sealed memfd through sudo and the mandatory installer lock exec, while a parent-death contract prevents descriptor-holder PID reuse. Privileged copies and installed snapshots never reopen staged package content by pathname. A successful or superseded installer marks only the exact validated `<sha>/<package>` source drained by atomically replacing `RUNBOOK.txt` with `DRAINED.txt`, never deleting its own execution tree. If receipt publication succeeds but marker draining is interrupted, a subsequent post-merge run validates the exact installed receipt and safely reconciles the lingering mode-0600 marker; unsafe marker metadata fails closed with a next action. A missing receipt/snapshot or non-equivalent divergent, modified, or unverifiable package fails closed. Post-merge deployment never republishes installed source after the owning installer releases its lock and never bulk-deletes deferrals from other SHAs. Recheck these guarantees with `/usr/local/sbin/hapax-root-required-deploy-audit`, `/usr/local/sbin/hapax-oom-policy-audit --json`, `uv run pytest -q tests/scripts/test_hapax_post_merge_deploy.py -k root_required_audit`, and `uv run pytest -q tests/scripts/test_hapax_oom_policy_audit.py -k package_lock`.
