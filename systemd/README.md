...
- **P0 OOM policy source package** (`scripts/install-p0-oom-containment`): validates the versioned per-host policy data and ownership manifest only. It performs no account lookup, live readback, root delegation, file installation, service activation, Docker mutation, or receipt advancement. A successful source check is not runtime or task-closure evidence.
  ...
  - Post-merge deployment may stage the exact changed OOM package and desired SHA as **non-authoritative desired-state evidence**. Its OOM `RUNBOOK.txt` contains no executable install command. Same-UID staged files and receipts are inputs to future work, never proof of runtime authority or completion.
  ...
  - Recurring enforcement is green only when `hapax-oom-score-enforce.timer` is loaded, enabled, and active and the enforcer service reports a successful result within 180 seconds. Current live scores cannot hide a stopped, disabled, failed, or stale timer.
  - `hapax-local-judge.service` is absent from the source unit directory so post-merge deployment cannot resurrect it. The generic user-unit installer owns its retirement transaction and leaves an installed `/dev/null` mask: it captures a full immutable container ID, disables and masks the historical unit before stopping its workload, removes wants/drop-ins, reconciles only that ID, and rejects a replacement identity. This source task does not authorize running the transaction on Appendix.
