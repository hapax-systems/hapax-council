...
- **P0 OOM containment package** (`scripts/install-p0-oom-containment`): installs source-controlled recovery policy into `/etc/default/earlyoom`, `/etc/systemd/system/*.service.d/oom-protect.conf`, `/etc/systemd/system/system.slice.d/oom-containment.conf`, `/etc/systemd/system/user.slice.d/oom-containment.conf`, `/etc/systemd/system/user-1000.slice.d/oom-containment.conf`, `/etc/systemd/system/user@1000.service.d/oom.conf`, `~/.config/systemd/user/{app,session}.slice.d/oom-containment.conf`, and the root-side `hapax-oom-score-enforce` service/timer plus the activation-independent `hapax-root-failure-intake@.service` bridge. It also owns and installs `hapax-local-judge.service`, the exact host-profile table, selected zram policy, `hapax-oom-policy-audit`, and `hapax-root-required-deploy-audit`; ordinary post-merge deployment therefore stages a changed judge unit with the root-required package, and `systemd/scripts/install-units.sh` skips it, so neither generic path copies, symlinks, enables, or restarts the unit. Recurring safety audits do not depend on a mutable source-activation alias. Both audit oneshots have a two-minute start deadline so a blocked package lock, user bus, UPS query, sudo probe, or Git read cannot suppress later timer recurrence indefinitely. The package is bound to account `hapax`, UID 1000, exact canonical host identities without suffix normalization, and fixed unit names; test overrides are refused for production install destinations.
  ...
  - Root-required post-merge deployment stages reconstructible exact-SHA packages at `~/.cache/hapax/post-merge-root-required/<sha>/`; desired SHAs, installed snapshots and receipts, and the shared lock remain under `~/.local/state/hapax/root-required/`. Desired state is recorded before any apply attempt and stays visibly pending until the installed receipt advances. Production Bash `--install` authority is retired: both package scripts refuse before setup, and `hapax-root-required-deferred-install` can execute them only in parent-selected isolated test mode. In production the exact-Git helper performs advisory narrowing, releases the caller-owned state lock, and invokes only `/usr/local/sbin/hapax-root-required-package-apply` through non-interactive sudo with the package enum and exact SHA. It passes no sealed FD, child result, task path, cap receipt, repository path, destination, or command plan as authority. The broker must be a single-link root-owned executable beneath root-owned non-writable ancestors and must independently derive semantic authority, canonical package bytes, fixed destinations, commands, live readback, receipt publication, and deferral drain in one privileged transaction. This source task does not install that trust root: an absent or rejected broker leaves `RUNBOOK.txt`, the desired receipt, and audit failure pending with an explicit separately runtime-authorized bootstrap action. Caller-owned task rows, cap receipts, sealed memfds, environment shape, call form, and user receipts are workflow evidence rather than same-UID security tokens. The recurring audits continue to bind the physical shared lock and exact installed snapshots. Isolated installer tests may use temporary destinations and fake mutators only beneath one authenticated scratch root; they are not a production authority path. Recheck with `uv run pytest -q tests/scripts/test_hapax_root_required_deferred_install.py`, `/usr/local/sbin/hapax-root-required-deploy-audit`, and `/usr/local/sbin/hapax-oom-policy-audit --json`. Runtime acceptance additionally requires the separately installed broker, exact installed receipts on both hosts, passing recurring audits, and the local-judge canary.
  - Broker trust classification: package and expected SHA are requests the broker independently derives and compares, not selectors it obeys. The broker must hold its own root-owned transaction lock; the caller-owned lock is advisory serialization and is released before broker invocation. Exact Git-bound snapshots and root-owned artifacts are evidence. Caller-owned `RUNBOOK.txt`, `DRAINED.txt`, task rows, cap receipts, and installed-receipt files remain workflow bookkeeping and cannot independently prove a privileged effect to a hostile same-UID process. Parent-selected isolated test mode is decided before the Bash child exists and confines all selected paths and fake executables below one authenticated scratch root. The separately bootstrapped production sudo policy must admit only the broker; the measured blanket `NOPASSWD: ALL` grant must be retired before this boundary is load-bearing.
