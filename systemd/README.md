...
- **Bounded per-host memory policy**: `config/root-required/oom-host-profiles.tsv`
  admits only `hapax-appendix` in the inclusive `59..61 GiB` interval and
  `hapax-podium` in the inclusive `123..125 GiB` interval. Canonical hostname
  and exactly one non-overlapping RAM interval must both match; larger memory
  variance, a DIMM loss, host rename, or new memory class fails closed.
  Appendix uses app `46G/54G`, UID
  `48G/56G`, and 16 GiB zram. Podium uses app `72G/88G`, UID `80G/96G`, and
  32 GiB zram. The installer copies the receipt-bound selected sources to the
  canonical destinations and rejects later drop-ins assigning governed keys.
  Both hosts' selected app/UID values live under symmetric
...
- **P0 OOM containment package** (`scripts/install-p0-oom-containment`): installs source-controlled recovery policy into `/etc/default/earlyoom`, `/etc/systemd/system/*.service.d/oom-protect.conf`, `/etc/systemd/system/system.slice.d/oom-containment.conf`, `/etc/systemd/system/user.slice.d/oom-containment.conf`, `/etc/systemd/system/user-1000.slice.d/oom-containment.conf`, `/etc/systemd/system/user@1000.service.d/oom.conf`, `~/.config/systemd/user/{app,session}.slice.d/oom-containment.conf`, and the root-side `hapax-oom-score-enforce` service/timer plus the activation-independent `hapax-root-failure-intake@.service` bridge. It also owns and installs `hapax-local-judge.service`, the exact host-profile table, selected zram policy, `hapax-oom-policy-audit`, and `hapax-root-required-deploy-audit`; ordinary post-merge deployment therefore stages a changed judge unit with the root-required package, and `systemd/scripts/install-units.sh` skips it, so neither generic path copies, symlinks, enables, or restarts the unit. Recurring safety audits do not depend on a mutable source-activation alias. Both audit oneshots have a two-minute start deadline so a blocked package lock, user bus, UPS query, sudo probe, or Git read cannot suppress later timer recurrence indefinitely. The package is bound to account `hapax`, UID 1000, exact canonical host identities without suffix normalization, and fixed unit names; test overrides are refused for production install destinations.
  - Host selection binds the exact canonical hostname to an explicit reviewed inclusive integer-`MemTotal`-GiB interval (`hapax-appendix/59..61`, `hapax-podium/123..125`). Every configured hard ceiling must remain below the interval minimum. A larger variance, overlapping table entry, or changed host identity fails closed instead of silently selecting policy for different hardware; repair inventory or add a separately reviewed non-overlapping interval.
  - Override closure: installer and audits scan the complete system and user unit search paths, including full unit/template fragments, inherited type/template drop-in directories, and persistent, runtime, and transient control roots, for all five governed memory keys. They also require `NeedDaemonReload=no` and inspect each manager's authoritative `FragmentPath` and `DropInPaths`, so stale loaded policy or a root omitted from a filesystem reconstruction cannot become a false green. An authorized repair removes known legacy host-memory drop-ins and exact persistent/runtime files created by `systemctl set-property`; transient live activation is followed by another scrub and manager reload. At steady state, only the receipt-bound canonical file may assign governed memory keys for each of the six governed slices/services. A higher-priority zram-generator main config such as `/run/systemd/zram-generator.conf` is audit-failing because it can shadow the installed `/etc` policy.
  ...
  - `user-1000.slice` and `user@1000.service`: host-selected `MemoryHigh/MemoryMax` is `48G/56G` on appendix and `80G/96G` on podium; both retain `MemorySwapMax=8G`, `MemoryLow=20G`, and `MemoryMin=10G`. `user@1000.service` also carries `OOMScoreAdjust=100` and `OOMPolicy=continue`, so the whole user manager does not shield every workload or stop after a descendant OOM kill.
  ...
  - `app.slice`: host-selected `MemoryHigh/MemoryMax` is `46G/54G` on appendix and `72G/88G` on podium; both retain `MemorySwapMax=8G`, `MemoryLow=16G`, and `MemoryMin=8G`. Broadcast-critical members carry leaf reservations, and the recurring audit rejects ancestor-floor dilution. Live activation does not restart the slice; its transient runtime write is scrubbed before the canonical file is reloaded and verified.
  - `session.slice`: `MemoryHigh`, `MemoryMax`, and `MemorySwapMax` remain `infinity`, while `MemoryLow=2G` and `MemoryMin=1G` cover the aggregate 1.5G/768M audio leaf floors for PipeWire, PipeWire Pulse, and WirePlumber. The recurring audit also sums every immediate child floor beneath `session.slice`, so a new session sibling cannot dilute those audio floors unnoticed. This modest floor does not exempt the rest of the interactive session from the host-selected UID hard cap (`56G` on appendix, `96G` on podium) or 8G UID swap cap. Activation does not restart the slice or audio services; only the canonical receipt-owned drop-in remains afterward.
  - Both profiles keep every hard ceiling below physical RAM. The UID hard stop includes sibling session scopes; `MemoryHigh` starts reclaim before the corresponding hard stop, and `MemorySwapMax=8G` prevents recurrence of the recorded swap exhaustion.
  - Docker containment: every existing `hapax-github-mcp-*` container is converged to `512M` RAM / `768M` total memory+swap. The receipt-owned judge source carries a candidate `4G` RAM / `6G` total memory+swap cap, but source acceptance does not establish runtime suitability; a separately authorized load canary must record peak cgroup memory and stable health/restart/OOM counters before activation or runtime closure. No target uses `--oom-kill-disable`, and both installer and recurring audit inspect running, stopped, and `Created` containers by stable full container ID. The recurring audit additionally requires a present `hapax-local-judge` container to be `running`, so an exited same-name container that would force a permanent unit restart loop is a loud gap with its immutable ID. They also require the judge's user manager to report `NeedDaemonReload=no`, the receipt-owned `FragmentPath`, no `DropInPaths`, and an effective `ExecStart` containing the candidate `4G` / `6G` limits. Recheck with `systemctl --user show hapax-local-judge.service -p NeedDaemonReload -p FragmentPath -p DropInPaths -p ExecStart --no-pager`; after the authorized activation, require `NeedDaemonReload=no`, `FragmentPath=/home/hapax/.config/systemd/user/hapax-local-judge.service`, an empty `DropInPaths`, and both Docker flags in `ExecStart`. A failed update or inspect may be treated as an ephemeral disappearance only after an unfiltered successful enumeration proves that both the exact ID and the governed container name are absent; renaming the original ID or replacing it under the same governed name therefore remains a hard failure.
  - `ManagedOOMMemoryPressure=kill` is deliberately deferred. Both hosts currently leave it at `auto` with `systemd-oomd` disabled/inactive; oomd would select a descendant cgroup and SIGKILL every process in it. Recheck with `systemctl show system.slice user.slice user-1000.slice user@1000.service -p ManagedOOMMemoryPressure -p ManagedOOMSwap --no-pager` and `systemctl is-enabled systemd-oomd.service; systemctl is-active systemd-oomd.service`. A separately authorized `--dry-run` pressure exercise must prove victim selection and `ManagedOOMPreference` behavior before activation can be called earlier and gentler.
  - Recheck: `scripts/install-p0-oom-containment --check`, `scripts/install-p0-oom-containment --verify-live`, `/usr/local/sbin/hapax-oom-policy-audit --json`, `systemctl show user@1000.service -p OOMScoreAdjust -p OOMPolicy -p MainPID`, `systemctl is-enabled hapax-oom-score-enforce.timer earlyoom.service`, `systemctl is-active hapax-oom-score-enforce.timer earlyoom.service`, and `systemctl show hapax-oom-score-enforce.service -p TimeoutStartUSec -p Result -p ExecMainStartTimestamp`. Confirm the system-manager bridge with `systemctl cat hapax-root-failure-intake@.service` and `systemctl show 'hapax-root-failure-intake@hapax-oom-score-enforce.service.service' -p User -p StartLimitIntervalUSec -p StartLimitBurst` after a governed failure witness. Also require `systemctl --user is-enabled hapax-oom-policy-audit.timer hapax-root-required-deploy-audit.timer`, `systemctl --user is-active hapax-oom-policy-audit.timer hapax-root-required-deploy-audit.timer`, and `systemctl --user show hapax-oom-policy-audit.service hapax-root-required-deploy-audit.service -p TimeoutStartUSec` with both services loaded at `2min`. Runtime incident closure is unavailable in this source revision: caller-owned desired or installed receipts, their mtimes, worktree state, and incident logs cannot attest a host-root package generation. A future closure check must begin its 15-minute no-recurrence window from matching cryptographically attested per-request and current-generation records bound to the exact package request ID and SHA, then require fingerprint `technical_alert:podium-oom-collapse-with-false-ups-failure-alert` to remain at baseline `count=2`, `last_seen=2026-07-10T02:15:38Z`, and `recurrence_count=0`.
  - Root-required post-merge deployment stages reconstructible exact-SHA packages at `~/.cache/hapax/post-merge-root-required/<sha>/`; desired SHAs and the shared caller lock remain under `~/.local/state/hapax/root-required/`. Production Bash `--install` authority is retired: both package scripts refuse before setup, and isolated test mode rejects root, mismatched real/effective/saved/filesystem IDs, effective capabilities, non-initial UID/GID maps, bind-mounted script aliases, and legacy installer-sudo selectors before external commands. The deferred helper contains no executable production broker, sudo, or completion path and refuses before state locking. Root ownership observed inside a process is namespace-relative: a same-UID process can create nested user and mount namespaces and counterfeit apparently fixed root-owned paths and unsigned receipts without gaining real host root. Ordinary owner/mode/dirfd checks are therefore defense-in-depth shape checks, not host-root authority. `RUNBOOK.txt`, logs, task rows, cap receipts, sealed memfds, snapshots, drained markers, and user receipts are caller bookkeeping and never completion proof. Recheck with `uv run pytest -q tests/scripts/test_hapax_root_required_deferred_install.py`, `/usr/local/sbin/hapax-root-required-deploy-audit`, and `/usr/local/sbin/hapax-oom-policy-audit --json`. Runtime acceptance is unavailable in this source revision.
  - Future broker requirements: package, expected SHA, and correlation ID are bounded requests the broker independently derives or validates, not selectors it obeys. A separately reviewed successor may enable production only after the broker signs or MACs request ID, package, SHA, and completion generation with a host-root-held key and the exact-Git helper verifies that attestation against a source-pinned trust anchor. The broker must also hold its root transaction lock across authority revalidation, effects, live readback, caller bookkeeping, drain, exclusive durable per-request publication, atomic durable current-generation replacement, and bounded retention. Successful diagnostic output must include the exact request ID; durable verification must use matching attested per-request and current records, never only `AUTHENTICATED-INSTALL.log` or a caller receipt. Separate runtime authority must cover the broker/key lifecycle, exact broker-only `NOSETENV` sudo policy, and retirement of blanket `NOPASSWD: ALL`. Separately reviewed exact `hapax-oom-score-enforce` trigger and read-only audit grants remain distinct; they cannot apply a package.
...
- **UPS power-alert provenance** (`scripts/install-apcupsd-power-alerts`): installs the apcupsd config/hooks/helper into `/etc/apcupsd`, hard-codes production hooks to `/etc/apcupsd/hapax-power-event.py`, installs `/etc/logrotate.d/hapax-ups-power-events` for `/var/log/hapax/ups-power-events.jsonl`, and enables/starts `apcupsd.service` so the sole shutdown-policy owner survives reboot; live verification requires both enabled and active state. The logrotate stanza uses `su root root` because `/var/log/hapax` is group-writable for local audit writers, atomically renames and recreates the live file as `0640 root:hapax`, and delays compression for one rotation so a helper that opened the old inode before rename can finish appending to the retained `.1` file without losing the receipt. A concurrent real-logrotate regression holds such an in-flight writer across rotation and requires the complete pre-rotation and post-rotation event partition. Battery-transfer receipts say only that the transfer event itself does not request shutdown (`event_requests_shutdown=false`), while global `shutdown_requested` remains unknown; restoration receipts are also neutral because either event can follow a real `doshutdown`. All three hooks fail open so telemetry failure cannot suppress or indefinitely delay apccontrol: `onbattery` and `offbattery` have a hard ten-second whole-helper deadline, while the dedicated `doshutdown` hook has a hard five-second deadline and attempts to record both shutdown fields as true. A blocked helper may therefore complete without a receipt. On the shutdown path, apcaccess and ntfy also have one-second internal ceilings.
  ...
  - Recheck: `scripts/install-apcupsd-power-alerts --check` and `scripts/install-apcupsd-power-alerts --verify-live`; repair live drift through a runtime-authorized `hapax-post-merge-deploy` package reconciliation, never a bare installer `--install`. Root invocation resolves podium UID 1000's home/group and returns durable state ownership to `hapax`; target overrides support tests and state-path recovery, not a portable alternate-account installation.
...
source. The same exclusion covers `hapax-local-judge.service`; the generic
installer must never replace its receipt-owned regular file with a checkout
symlink. System-scoped units marked
