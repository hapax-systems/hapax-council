...
- **P0 OOM containment package** (`scripts/install-p0-oom-containment`): installs source-controlled recovery policy into `/etc/default/earlyoom`, `/etc/systemd/system/*.service.d/oom-protect.conf`, `/etc/systemd/system/system.slice.d/oom-containment.conf`, `/etc/systemd/system/user-1000.slice.d/oom-containment.conf`, `/etc/systemd/system/user@1000.service.d/oom.conf`, `~/.config/systemd/user/app.slice.d/oom-containment.conf`, and the root-side `hapax-oom-score-enforce` service/timer plus the activation-independent `hapax-root-failure-intake@.service` bridge. Root invocation still targets UID 1000's home; override both `HAPAX_OOM_TARGET_UID` and `HAPAX_OOM_TARGET_HOME` for a different operator account.
  ...
  - `system.slice`: `MemoryLow=24G` and `MemoryMin=12G`, with no hard ceiling, reciprocally reserve the measured recovery-plane footprint. At the 2026-07-09 recurrence review it used 24.6 GiB and had peaked at 28.8 GiB.
  ...
  - Root enforcer failures route to `/usr/local/sbin/hapax-root-failure-intake`, which calls the stable D2 recovery bundle under `~/.local/lib/hapax-recovery/council/current` and records an emergency JSONL fallback if that bundle is missing. The failure bridge admits at most one start per hour, preventing a persistent 30-second enforcer failure from creating an incident storm while leaving successful timer runs unrestricted.
  ...
  - Recheck: `scripts/install-p0-oom-containment --check`, `scripts/install-p0-oom-containment --verify-live`, `scripts/hapax-oom-policy-audit --json`, `systemctl is-enabled hapax-oom-score-enforce.timer`, `systemctl is-active hapax-oom-score-enforce.timer`, and `systemctl show hapax-oom-score-enforce.service -p Result -p ExecMainStartTimestamp`. Also require `systemctl --user is-enabled hapax-oom-policy-audit.timer hapax-root-required-deploy-audit.timer` and `systemctl --user is-active hapax-oom-policy-audit.timer hapax-root-required-deploy-audit.timer`. Inspect recurrence state with `jq . ~/.cache/hapax/p0-incident-intake/state.json` and `tail -n 20 ~/.cache/hapax/p0-incident-intake/events.jsonl`.
  - Root-required post-merge installers return rc `77` when non-interactive sudo is unavailable; `hapax-post-merge-deploy` stages those packages under `~/.cache/hapax/post-merge-root-required/<sha>/`. Run the staged RUNBOOK command after `sudo -v`; a successful installer records its owned source files under `~/.cache/hapax/post-merge-root-required/current-source` and drains only the explicit `HAPAX_ROOT_REQUIRED_DRAIN_DIR`. A later successful `hapax-post-merge-deploy` for the same package may clear older same-package deferrals after installing the newest staged package.
