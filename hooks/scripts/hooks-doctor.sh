...
CLOSURE_SIBLINGS=(agent-role.sh escape-grant.sh cc-task-root.sh hapax_check_enable_latch.sh cc-task-gate-bootstrap.py hooks-doctor.sh)
...
check_canonical() {
  ...
  # The impl SOURCES agent-role.sh + escape-grant.sh + cc-task-root.sh and INVOKES
  # cc-task-gate-bootstrap.py (section 3b) — all from its own dir. A missing
  # required sibling must be loud here so deploy cannot silently drop it.
