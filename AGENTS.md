...
- **One task at a time — but "active" means WORKER-HELD, not merely non-terminal.** `cc-claim` refuses a new task while the current claim is `claimed`, `in_progress` or `blocked`. It does **not** refuse when the current task has released the role: terminal statuses, and also the **pipeline-held** ones (`pr_open`, `merge_queue`, `ci_green`, the ready-family, `merged_awaiting_runtime_witness`, `backlog`). Those are non-terminal and the lane is still free, because the work is finished and the system owes a verdict — counting them against a worker cap measured 43 lanes held hostage by work awaiting review, which is a feedback loop rather than a capacity limit. The vocabulary is `shared/sdlc_lifecycle.TASK_ROLE_RELEASING_STATUSES`; a released task's lease sidecars are archived to its lineage, under the role publication lock, before the next claim publishes. Use `cc-close` for a task that is genuinely done, or the governed stale-lease release procedure for a stale claim. The admitted default path rejects `cc-claim --force`; use the explicit emergency fallback only with operator authorization.
