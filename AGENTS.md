...
- Prefer the common visible-lane launcher, `scripts/hapax-dev --runtime codex --role cx-<color> --task <cc-task-id>` or `scripts/hapax-dev --runtime claude --role <role> --task <cc-task-id>`, so ROLE and work/dispatch capability are established through one interface. `scripts/hapax-codex` and `scripts/hapax-claude` remain the underlying platform launchers. Without `--cd`, non-primary Codex sessions use Codex-native worktrees named `~/projects/hapax-council--cx-<color>`.
