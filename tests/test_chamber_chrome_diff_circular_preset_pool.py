"""Regression pin for the chamber/chrome/diff-motion/circular preset pool.

7th pool in the operator-directed continuation train.
"""
...
EXPECTED_PRESETS: tuple[tuple[str, str], ...] = (
    ("chamber_feedback_dense", "chamber-feedback"),
    ("chamber_feedback_breathing", "chamber-feedback"),
    ("chrome_mirror_polished", "chrome-mirror"),
    ("chrome_mirror_brushed", "chrome-mirror"),
    ("diff_motion_trail", "diff-motion"),
    ("diff_motion_thermal", "diff-motion"),
    ("circular_lens_focus", "circular-mask-focus"),
    ("circular_porthole_view", "circular-mask-focus"),
)
...
EXPECTED_LINEAGE_COUNTS: dict[str, int] = {
    ...
    "chrome-mirror": 2,
    "diff-motion": 2,
    "circular-mask-focus": 2,
}

# 48 names from prior 6 pools.
PRIOR_POOLS: frozenset[str] = frozenset(
    {
        ...
        # kaleido-pixsort-chromakey-sierpinski (#2415)
...
class TestLineageDistinctness:
    LINEAGE_PRIMARY_NODES: dict[str, frozenset[str]] = {
        "chamber-feedback": frozenset({"feedback", "trail", "echo"}),
        "chrome-mirror": frozenset({"mirror", "emboss"}),
        "diff-motion": frozenset({"diff"}),
        "circular-mask-focus": frozenset({"circular_mask"}),
    }
