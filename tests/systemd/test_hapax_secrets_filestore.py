...
# The common file has carried the signing key as an OPTIONAL entry since #4680; the
# dedicated projection must leave that common projection exactly as it was.
COMMON_WITH_AUTHORITY = COMMON_BASELINE.replace(
    b"HAPAX_OPERATOR_ORCID=0000-0000-0000-0000\n",
    b"HAPAX_OPERATOR_ORCID=0000-0000-0000-0000\n"
    + AUTHORITY_ENV.encode()
    + b"="
    + AUTHORITY_VALUE
    + b"\n",
)
...
def test_producer_dedicated_projection_leaves_common_projection_unchanged(producer_store) -> None:
    store, common = producer_store
    ...
    result = _run_producer(os.environ.copy())
    assert result.returncode == 0, result.stderr
    assert common.read_bytes() == COMMON_WITH_AUTHORITY


@pytest.mark.parametrize("empty", [b"", b"\n", b"   \n", b"\nsecond-line"])
def test_producer_empty_authority_never_replaces_working_key(producer_store, empty) -> None:
    store, common = producer_store
    store.put(AUTHORITY_ENTRY, empty)
    authority = common.with_name(AUTHORITY_FILE)
    working = AUTHORITY_ENV.encode() + b"=" + AUTHORITY_VALUE + b"\n"
    common.write_bytes(COMMON_WITH_AUTHORITY)
    authority.write_bytes(working)
    inodes = (common.stat().st_ino, authority.stat().st_ino)
    result = _run_producer(os.environ.copy())
    assert result.returncode == 2, result.stderr
    assert f"FileStore entry {AUTHORITY_ENTRY} is empty" in result.stderr
    assert "Next action:" in result.stderr
    assert "re-put" in result.stderr
    assert "wrote" not in result.stdout
    assert authority.read_bytes() == working
    assert common.read_bytes() == COMMON_WITH_AUTHORITY
    assert (common.stat().st_ino, authority.stat().st_ino) == inodes
    assert not list(common.parent.glob(".*.tmp"))


@pytest.mark.parametrize("shape", ["symlink-loop", "directory", "dangling-symlink"])
def test_producer_undemonstrated_absence_keeps_authority_file(producer_store, shape) -> None:
    # has() is Path.is_file(), which is False for each of these without the entry being
    # absent; only FileNotFoundError on the entry's own path may remove the signing file.
    ...
    authority.write_bytes(working)
    inodes = (common.stat().st_ino, authority.stat().st_ino)
    blob = common.parent / "secrets" / f"{AUTHORITY_ENTRY}.bin"
    if shape == "symlink-loop":
        blob.symlink_to(blob.name)
    elif shape == "directory":
        blob.mkdir()
    else:
        blob.symlink_to("missing-target.bin")
    result = _run_producer(os.environ.copy())
    assert result.returncode == 2, result.stderr
    assert f"FileStore entry {AUTHORITY_ENTRY} could not be shown absent" in result.stderr
    assert "Next action:" in result.stderr
    assert "wrote" not in result.stdout
    assert authority.read_bytes() == working
    assert common.read_bytes() == COMMON_WITH_AUTHORITY
...
def test_producer_mid_write_failure_preserves_final_file(
    ...
    with pytest.raises(OSError, match="synthetic mid-write failure"):
        runpy.run_path(str(PRODUCER), run_name="__main__")
    # The success line reports both publications, so it must not precede a failed one.
