...
def test_obs_v4l2_source_reset_runs_from_activation_worktree_with_notify_watchdog() -> None:
    ...
    assert "--screenshot-width 320" in parser.get("Service", "ExecStart")
    assert "--screenshot-height 180" in parser.get("Service", "ExecStart")
    assert "hapax-compositor-runtime-source-check" in parser.get("Service", "ExecStartPre")
...
def test_screwm_obs_v4l2_reset_dropin_pins_obs_to_direct_video50() -> None:
    ...
    assert "--poll-interval 5" in joined
    ...
    assert "--producer-restart-after-obs-resets 1" in joined
    assert "--max-static-ignore-seconds 45" in joined
    assert "--screenshot-width 320" in joined
    assert "--screenshot-height 180" in joined
