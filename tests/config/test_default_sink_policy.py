...
RYZEN_SINK = "alsa_output.pci-0000_73_00.6.analog-stereo"
FAIL_CLOSED_DEFAULT_SINK = "hapax-pc-loudnorm"
MPC_SINK = "alsa_output.usb-Akai_Professional_MPC_LIVE_III"


def test_wireplumber_default_sink_policy_prefers_fail_closed_and_deprioritizes_hardware() -> None:
    text = DEFAULT_SINK_CONF.read_text(encoding="utf-8")
    assert FAIL_CLOSED_DEFAULT_SINK in text
    assert RYZEN_SINK in text
    assert L12_SINK in text
    assert MPC_SINK in text
    assert "priority.session = 2200" in text
    assert "priority.session = 10" in text
    assert "NOT on L-12, MPC" in text
    assert "Every other workstation/default/multimedia path is under-specified" in text


def test_usb_witness_does_not_restore_l12_as_default_sink() -> None:
    text = USB_WITNESS.read_text(encoding="utf-8")
    assert 'FAIL_CLOSED_DEFAULT_SINK = "hapax-pc-loudnorm"' in text
    assert "fail_closed_default_sink_restored" in text
    assert "local_default_sink_restored" not in text
...
def test_usb_runbook_matches_default_sink_policy() -> None:
    ...
    assert "default sink is the fail-closed" in text
    assert "not any physical hardware" in text
