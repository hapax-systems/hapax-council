...
returns, prints, or logs the contents of a ``.bin`` blob. These tests
plant a sentinel byte string in fake ``.bin`` blobs and assert the
...
from agents.hapax_cred_monitor.monitor import compute_delta, walk_secret_store
...
def _make_store_with_sentinels(root: Path, entries: list[str]) -> Path:
    ...
    for name in entries:
        (store / f"{name}.bin").write_bytes(SENTINEL)
    return store
...
class TestNoValueLeakage:
    ...
    def test_arrival_log_never_contains_sentinel(self, tmp_path: Path) -> None:
        ...
        snap_a = walk_secret_store(store_a)
        snap_b = walk_secret_store(store_b)
