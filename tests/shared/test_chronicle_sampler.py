...
class TestStimmungReader:
    ...
    def test_nested_dimensions_passed_through(self, tmp_path: Path) -> None:
        path = tmp_path / "s.json"
        path.write_text(
            json.dumps({"dimensions": {"health": {"value": 0.9}, "operator_energy": 0.5}})
        )
        result = assemble_snapshot(stimmung_path=path, eigenform_path=tmp_path / "no.jsonl")
    ...
    def test_top_level_dim_dict_value_extracted(self, tmp_path: Path) -> None:
        ...
        path = tmp_path / "s.json"
        path.write_text(json.dumps({"health": {"value": 0.85, "trend": "rising"}}))
        result = assemble_snapshot(stimmung_path=path, eigenform_path=tmp_path / "no.jsonl")
...
class TestEigenformReader:
    def test_returns_last_entry(self, tmp_path: Path) -> None:
        ...
        result = assemble_snapshot(stimmung_path=tmp_path / "no.json", eigenform_path=path)
