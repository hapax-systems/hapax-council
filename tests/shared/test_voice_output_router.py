...
def _write_private_status(
    ...
    *,
    state: str = "ready",
    reason_code: str = "mk5_private_monitor_bound",
) -> None:
    payload = {
        "bridge": {
            "installed": True,
        ...
        "operator_visible_reason": (
            ...
            if state == "ready"
            else "mk5 Phones private monitor target is absent; private monitor route remains silent."
        ),
...
def test_private_assistant_route_requires_fresh_exact_monitor_evidence(tmp_path: Path) -> None:
    ...
    assert result.target_binding.target_ref == "audio.mk5_private_monitor"
...
def test_private_blocked_absent_status_carries_mk5_reason(tmp_path: Path) -> None:
    status_path = tmp_path / "private-monitor-target.json"
    _write_private_status(
        status_path,
        state="blocked_absent",
        reason_code="mk5_private_monitor_target_absent",
    )
    ...
    assert result.state is VoiceRouteState.BLOCKED
    assert result.reason_code == "mk5_private_monitor_target_absent"
    assert result.target_binding is not None
    assert result.target_binding.target is None
