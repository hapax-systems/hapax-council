...
def test_private_sink_monitor_is_captured_and_played_to_yeti_only() -> None:
    ...
    assert f'target.object = "{YETI_TARGET}"' in playback


def test_notification_private_sink_monitor_is_captured_and_played_to_yeti_only() -> None:
    active = _active(_conf())
...
def test_playback_streams_are_fail_closed_when_yeti_is_absent() -> None:
    active = _active(_conf())
