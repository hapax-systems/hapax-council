"""The coordinator observes target-host pressure without using it as candidate authority.

Regression for the wrong-host observation bug: ``Coordinator.tick`` called the
pressure gate with no ``target_host``, so appendix-bound SDLC dispatch was
...
(``LOCAL_DEV_TARGET``); support observation must read that host without persisting.
"""
...
def test_tick_observes_the_dispatch_target_not_local(tmp_path: Path) -> None:
    coord = Coordinator()
    with (
        patch.object(coord, "_scan_tasks", return_value=[]),
        patch.object(coord, "_check_lanes", return_value={}),
        patch("agents.coordinator.core.observe_admission_state") as mock_admission,
        patch("agents.coordinator.core.SHM_DIR", tmp_path / "shm"),
        patch("agents.coordinator.core.SHM_FILE", tmp_path / "shm" / "state.json"),
        patch("subprocess.run"),
    ):
        # Candidate planning remains empty; this isolates the support read itself.
