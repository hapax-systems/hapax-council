...
def test_no_claim_blocks_task_bootstrap_without_claimable(tmp_path: Path) -> None:
    """A note born `offered` + `unassigned` but without `claimable` cannot be claimed.

    The bootstrap validator let rows be minted without the field that cc-claim's
    publication path requires for a fresh claim, so the row arrived in the offered
    queue already unclaimable. Requiring it at birth is the cheaper half of the fix.
    """
    request_root = tmp_path / "Documents/Personal/20-projects/hapax-requests/active"
    request_root.mkdir(parents=True)
    request_path = request_root / "REQ-20260517150000-perspective-merge-remediation.md"
