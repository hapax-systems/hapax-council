...
# and asserts the branch was pushed to the bare remote.
set -euo pipefail
...
if [ "$REMOTE_SHA" = "$LOCAL_SHA" ]; then
  echo "PASS: remote ref matches local HEAD after worktree commit"
else
  echo "FAIL: remote=$REMOTE_SHA local=$LOCAL_SHA" >&2
  exit 1
fi
