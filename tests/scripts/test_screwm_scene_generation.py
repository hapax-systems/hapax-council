...
def test_screwm_map_spatializes_all_wards_as_geometric_instruments() -> None:
    ...
    assert content.count("// ward-garden-drift-stone ") == len(module["DRIFT_LINKS"])
    ...
    assert content.count("// scroom-garden-lantern-cap ") == len(module["SCROOM_GARDEN_LANTERNS"])
    ...
    assert "// ward-garden-pane 01: token_pole ward_atlas" in content
    assert "// ward-garden-pane 35: m8_oscilloscope ward_atlas" in content
    ...
    assert module["ward_live_mount_contract"](2, "album")["source_aspect"] == [10, 13]
    assert module["ward_live_mount_contract"](5, "reverie")["texture"] == "w05"
    assert module["ward_garden_facing"](5) == "y"
    assert module["ward_live_mount_contract"](8, "gem")["source_aspect"] == [23, 3]
    ...
    assert module["static_ward_mount_contract"](1, "token_pole", "drift_c")[
        "material_profile"
    ] == "state-ward-instrument"
    assert module["ward_review_drift_midpoint"](1, 9) == (-710, -2360, 120)
...
def test_screwm_room_textures_are_information_surfaces_not_identifiable_materials() -> None:
    ...
    surface_contracts = json.loads(
        (REPO_ROOT / "config" / "screwm-quake-surface-contracts.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_screwm_review_geometry_keeps_wards_primary_not_architecture() -> None:
    ...
    assert '"_minlight" "6"' in source
    assert '"_minlight_color" "0.07 0.08 0.11"' in source
...
def test_screwm_live_media_panes_have_one_truth_bearing_face() -> None:
    module = _load_script("scripts/generate-screwm-map.py")
    content = module["generate_map"](module["MODE_PRESETS"]["rnd"])
    ...
    for idx, source in enumerate(module["SOURCE_ANCHORS"], start=1):
        role = source["role"]
        tex = source["texture"]
        source_pane = _comment_block(
            content, f"// source-garden-anchor {idx:02d}: {role} {tex}"
        )
    ...
    for idx, tex in ((9, "w09"), (22, "w22"), (27, "w27")):
        ...
        ticker_pane = _comment_block(
            content, f"// ward-garden-pane {idx:02d}: {name} {tex}"
        )
...
def test_live_media_textures_are_self_lit_information_surfaces() -> None:
    ...
    for name in live_names:
        ...
        next_block = min(
            [idx for idx in (shader.find(f"\n{candidate}\n{{", block_start + 1) for candidate in live_names) if idx != -1]
            or [len(shader)]
        )
...
def test_aoa_model_transform_stands_pyramid_upright_and_centers_media_front() -> None:
    ...
    world_sphere_radius = (
        module["ATTENDANT_SPHERE_RADIUS"]
        * module["SCALE"]
        * module["AOA_SPHERE_MODEL_SCALE"]
    )
    assert abs(round(derived_scale, 2) - 3.87) < 0.001
    assert abs(
        world_inradius - world_sphere_radius * module["ATTENDANT_SPHERE_CLEARANCE_RATIO"]
    ) < 0.001
...
def test_screwm_wad_defines_only_declared_live_ward_receiver_textures() -> None:
    ...
    assert {
        textures[name]["pattern"] for name in default_surface_textures
    }.isdisjoint(identifiable_material_patterns)
