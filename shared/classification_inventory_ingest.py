...
def _descriptor_from_row(row: dict[str, object]) -> CapabilityHarnessDescriptor | None:
    ...
    if "public" in ceiling_str or "publish" in ceiling_str:
        authority = AuthorityCeiling.PUBLIC_PUBLISH
    elif "mutate" in ceiling_str or "repo" in ceiling_str:
        authority = AuthorityCeiling.REPO_MUTATION
    else:
        authority = AuthorityCeiling.READ_ONLY
