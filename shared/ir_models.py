...
class HandSemantics(BaseModel):
    ...
    Schema mirrors ``shared.ir_vlm_classifier.HandSemantics`` but with
    permissive defaults so reports from older Pi-edge daemons that
    don't carry ``hand_semantics`` deserialize without error.
    """
