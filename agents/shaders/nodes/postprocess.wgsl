...
fn main_1() {
    ...
    if mediation > 0.001 {
        ...
        let n = hash(v_texcoord_1 * vec2<f32>(270.0, 180.0) + c.rg * 3.0 + vec2<f32>(17.0, 0.0));
        let scan = sin((v_texcoord_1.y * 720.0) * 1.35) * 0.5 + 0.5;
        let veil = vec3<f32>(
            n - 0.5,
            hash(v_texcoord_1 * 330.0 + vec2<f32>(11.7, 29.3)) - 0.5,
            scan - 0.5,
        );
