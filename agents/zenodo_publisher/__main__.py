"""One-shot CLI for the Zenodo publisher.

Usage::

    uv run python -m agents.zenodo_publisher --slug <slug>

Loads a single ``PreprintArtifact`` from
``$HAPAX_STATE/publish/inbox/{slug}.json`` and dispatches it to the
Zenodo REST API. Production dispatch goes through
``agents.publish_orchestrator``; this CLI is for manual single-artifact
debugging only.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="agents.zenodo_publisher",
        description="One-shot Zenodo dispatch for debugging.",
    )
