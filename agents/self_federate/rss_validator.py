"""Hapax weblog RSS feed validator — Phase 1.

Per cc-task ``cold-contact-activitypub-rss-self-federate``. Verifies
...
Phase 1 ships the validator + DOI extraction. Phase 2 wires the
weekly cadence (systemd timer) + ntfy on validity loss; deferred
Phase 3 adds Bridgy Fed bridge activation for ActivityPub.
...
def main() -> int:
    ...
    count + DOI cross-link coverage. Daemon-friendly: every failure
