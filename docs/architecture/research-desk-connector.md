...
**Status:** implemented, unregistered. The server, the units, the queue contract and the
ledger are in this repo and tested. Two acts remain, and both are the operator's: register the
connector in Perplexity, and create one scheduled task.
...
| element | value |
|---|---|
| public URL | `https://desk.hapaxrnd.com/mcp` |
| transport | MCP Streamable HTTP, **stateless** (each request independent — the client is a fresh agent every run) |
| origin bind | `127.0.0.1:8790`; a non-loopback bind is refused outright |
| health | `GET /healthz` → `{"ok": true, "service": "hapax-research-desk"}`, unauthenticated, no secrets |
| Host allowlist | loopback plus `desk.hapaxrnd.com` (`HAPAX_RESEARCH_DESK_PUBLIC_HOST`). DNS-rebinding protection stays **on** |
| auth | `Authorization: Bearer <key>` **or** `X-API-Key: <key>`, constant-time compared |
| key | FileStore name `research-desk-connector-key` |
| tunnel credentials | FileStore name `cloudflared-research-desk-tunnel-credentials`, materialised 0600 into `$XDG_RUNTIME_DIR/cloudflared/research-desk.json` at unit start and removed on stop |
...
2. **Citations.** `http`/`https` only — a `file:` or `javascript:` "citation" is a hazard, not a
   source, and it would be a live link in a vault reader.
3. **Labelling.** The drop's frontmatter is estate-authored and carries
...
| unit | what it is |
|---|---|
| `systemd/units/hapax-research-desk-mcp.service` | the server. `ExecStartPre` runs `--check`, which validates the FileStore credential and both directories — a desk that cannot authenticate never binds. `ProtectSystem=strict` with `ReadWritePaths` limited to the active-requests dir, the lanebus and the desk's own cache. |
| `systemd/units/hapax-research-desk-tunnel.service` | `cloudflared tunnel run`. `Requires=` the server: a live public hostname with no origin behind it is a 502 the connector learns as a dead server. `ConditionPathExists=` the ingress config, so it stays **inert** rather than failed until the coordinator's inputs exist. |
...
Ingress config (coordinator-owned, at `~/.cloudflared/research-desk.yml`, already in place):

```yaml
tunnel: fcbd6235-417e-48e1-96d7-4177e46e290b
