...
8. **Reviewer invocation** — family roster in the registry holds reviewer command argv templates: claude `claude -p`, codex `codex exec --sandbox read-only -`, gemini family `scripts/hapax-agy-reviewer` backed by route `agy.review.direct` when its route receipts admit it, and glm `scripts/hapax-glmcp-reviewer`. The dispatcher sends the blind-review payload to the configured command's stdin. The agy wrapper is the `agy.review.direct` read-only review-seat command behind the gemini family; it reads that dispatcher dossier from stdin, writes a sandbox-local `review-dossier.md`, and invokes `/usr/bin/agy --print <short prompt>` without subprocess stdin so agy reviews only that file. The GLM review adapter is a direct OpenAI-compatible Coding Plan caller that reads the prompt from stdin and is configured with `HAPAX_GLMCP_REVIEW_*` environment variables, separate from the Claude Code launcher `HAPAX_GLMCP_*` variables. Dispatcher abstracts both behind a `runner` callable so tests stub them. Recheck agy-backed gemini with `python -m py_compile scripts/hapax-agy-reviewer`, `uv run pytest tests/scripts/test_hapax_agy_reviewer.py -q`, `uv run pytest tests/test_review_team.py::test_gemini_review_family_uses_agy_wrapper_not_legacy_cli -q`, and `uv run pytest tests/test_cc_pr_review_dispatch.py::TestApply::test_blocked_agy_route_is_not_invoked_as_reviewer -q`; recheck GLM with `scripts/hapax-glmcp-reviewer --check` and `uv run pytest tests/scripts/test_hapax_glmcp_reviewer.py -q`.
...
- [ ] **Step 3: Write registry.yaml** with: `always_on_lenses`, `surface_lenses`
  ...
  `cx-glmcp`/`codex-glmcp`/`glmcp` and `glm-*`→glm,
...
- [ ] **Step 2-4: TDD until green.** Lane send: `scripts/hapax-{claude,codex}-send --session <lane> -- <short pointer to payload file>` chosen by lane_families; agy review seats are invoked through `scripts/hapax-agy-reviewer` and do not use lane-send. Failures are logged, never fatal.
