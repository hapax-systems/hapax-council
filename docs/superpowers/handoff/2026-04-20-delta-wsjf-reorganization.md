...
| **D-26** | **NEW** Phase 5 wire: replace `programme=None` at `shared/affordance_pipeline.py:434` with live active-programme lookup; ADD `monetization_opt_ins` field to Programme primitive + validation rules per `docs/superpowers/plans/2026-04-20-demonetization-safety-plan.md` §Phase 5 (lines 349-388) | Audit by alpha 2026-04-20T18:30Z verified `affordance_pipeline.py:430-431` is the explicit Phase-5 stub; demonet plan §Phase 5 specifies the full scope (150-250 LOC + ≥6 tests + validation) | 13 | 8 | 13 | 5 | **6.8** | READY | **PREREQUISITE for D-17, D-18, D-27** |
| **D-27** | **NEW** Egress audit wire: call `MonetizationEgressAudit.default_writer().record(...)` from `MonetizationRiskGate.assess()` per demonet plan §Phase 6 (currently writer is shipped but unwired — verified by alpha 2026-04-20T18:30Z) | Demonet plan lines 389-405; agent audit found `EgressAuditWriter` shipped + tested but `GATE.assess()` never calls it | 8 | 5 | 8 | 3 | **7.0** | READY | independent of D-26 (writer takes RiskAssessment, doesn't need Programme) |
