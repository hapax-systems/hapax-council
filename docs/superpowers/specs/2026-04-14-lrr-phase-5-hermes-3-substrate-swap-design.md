...
>
> **Complementary ratification artifact (PR #826, alpha):** the alpha session (owns `beta.yaml` per delta's 2026-04-15T05:05Z identity-correction inflection) independently received operator ratification at 04:40Z and shipped PR #826 (branch `docs/drop-62-option-c-ratification`) with a main-branch decision record at `docs/research/2026-04-15-drop-62-option-c-ratification.md`, draft systemd unit `systemd/units/tabbyapi-hermes8b.service`, GPU pin drop-in with operator-choice TODO, and embedded LiteLLM route drafts + `config.yml.hermes-8b` draft. Alpha's second commit on PR #826 (`77d20c80`) cross-references this spec's commits `738fde330` + `156beef92`. The two PRs are complementary and non-overlapping: PR #819 carries the Phase 5 spec + DEVIATION-037 amendment headers (this spec); PR #826 carries the main-branch decision record + repo scaffolding for 5a execution. A future reader finds the full ratification picture by following either cross-reference.
