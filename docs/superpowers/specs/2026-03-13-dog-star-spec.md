...
VoiceLifecycle.is_guest_mode → self.speaker not in ("operator", None)
  # Creates: operator ("operator") vs. non-operator (guest)

# speaker_id.py — biometric identity classification
SpeakerIdentifier.identify(audio) → SpeakerResult
  # Returns: "operator" (≥0.75), "not_operator" (<0.4), "uncertain"
...
Non-operator speaks → wake word triggers session
  ...
  → SpeakerResult(label="not_operator", confidence=0.85)
