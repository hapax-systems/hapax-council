...
The seat SessionStart hook matches `startup`, `resume`, and `clear`. It fires when `HAPAX_AGENT_ROLE` equals the process role named in the §0 incumbent row. It does not scan the task store. It reads `~/.cache/hapax/seat-owed-set-summary.txt`, states that file's age, and prints the full-list path. If the file is missing, it prints one unavailable line and that path. The existing 15-minute `lanebus-staleness-sweep` timer refreshes the file. The drop-in source is `systemd/units/lanebus-staleness-sweep.service.d/seat-owed-set.conf`. On merge the deployed copy replaces the interim live file and the `~/.local/bin/hapax-seat-owed-set` symlink points at the release tree. The bus inbox stays the `family/name` token on that row. The summary lists new rows and undisposed bus asks in full, then backlog counts per class and the oldest 15 backlog rows.
