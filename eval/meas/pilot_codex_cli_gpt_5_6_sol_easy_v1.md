...
workspace-write sandbox; both surfaces were removed from the shipped v12 driver.
...
This result predates the v12 external agent-read boundary, clean scoring checkout,
...
measurement of the shipped v12 driver. The adjacent witness contains the redacted
19-cell evidence, task and commit contracts, λ, result seals, and artifact seal.

No provider-backed 19-cell pass rate has been measured for v12. That measurement is
deferred to a separately authorized provider-spend follow-up tranche; the v1 rate
must not be used as a v12 performance claim. The provider-backed v12 measurement
remains explicit acceptance work for that separately authorized follow-up tranche.

## Shipped-driver validation

The provider-free v12 validation ran on `hapax-appendix` (Linux 6.18.32,
Bubblewrap 0.11.2, Codex CLI 0.148.0). Both optional integration prerequisites were
present: `uv run pytest -q tests/eval` reported **80 passed, 0 skipped**, and
...
v12 pass rate. On other hosts, the `requires_bubblewrap` and `requires_codex` markers
...
shipped v12 clean-checkout, read-only predicate, worker-integrity guard,
