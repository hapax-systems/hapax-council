...
workspace-write sandbox; both surfaces were removed from the shipped v9 driver.
...
This result predates the v9 external agent-read boundary, clean scoring checkout,
...
measurement of the shipped v9 driver. The adjacent witness contains the redacted
19-cell evidence, task and commit contracts, λ, result seals, and artifact seal.

No provider-backed 19-cell pass rate has been measured for v9. That measurement is
deferred to a separately authorized provider-spend follow-up tranche; the v1 rate
must not be used as a v9 performance claim.
...
shipped v9 clean-checkout, trusted-source provenance gate, merge-test installation,
isolated-worker, and trusted-controller path without provider spend, run:
