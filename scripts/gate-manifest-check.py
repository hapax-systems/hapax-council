...
def _check_codex_config(
    ...
    *,
    ...
    # Configuration lives in the physical release's shared helper. Check the
    # launcher still sources, calls and consumes that composition; runtime argv
    # behavior is exercised separately by the launcher integration tests.
    for marker, label in (
        ('. "$EXECUTION_SOURCE_ROOT/scripts/capability-execution.sh"', "helper source"),
        ('bind_codex_common_config "$HOME" "$WORKDIR" "$HOOK" "$LOGOS_BASE_URL"', "helper call"),
