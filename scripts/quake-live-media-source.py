...
def _resolve_youtube_page_url(args: argparse.Namespace) -> str:
    ...
    if configured_url == DEFAULT_YOUTUBE_URL and getattr(args, "youtube_fallback", "canary") == "offline":
...
def _write_meta(path: Path, args: argparse.Namespace, frames: int) -> None:
    payload = {
        ...
        "configured_url": getattr(args, "configured_url", args.url) if args.source == "youtube" else "",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sphere-front-aspect",
        type=float,
        default=float(os.environ.get("HAPAX_QUAKE_SPHERE_FRONT_ASPECT", DEFAULT_SPHERE_MEDIA_ASPECT)),
