...
MODEL = os.environ.get("DELIBERATION_MODEL", "fast")
...
ROUND_SYSTEM = """You are {agent} in round {round_num} of a governance deliberation. You have read the other agent's previous argument.
...
- You MUST check each of your pre-committed update conditions against the other agent's argument
- A condition is MET if the other agent's argument SUBSTANTIALLY ADDRESSES the concern behind the condition, even if they used a different argumentative path than the condition literally describes. Ask: "Did the other agent give me a reason to update this part of my position?" If yes, the condition is met.
- If a condition is met, you MUST add a corresponding entry to "concessions"
- If you are conceding a point for ANY reason, check whether any of your update conditions relate to that concession. If so, mark the condition as met. Concessions and update conditions should be LINKED — a concession without a corresponding met condition (when one exists) indicates the checking is too strict.
- Be HONEST. Do not rationalize away strong arguments. If you find yourself writing "not met" but also conceding a related point, something is wrong — reconsider whether the condition is met.

RULES FOR CONCESSIONS:
- Concessions must be SPECIFIC governance claims, not vague acknowledgments like "Brutus raises a valid point"
...
def run_deliberation(tension: dict) -> dict:
    ...
    brutus_r2_raw = llm_call(
        [
            {
                "role": "system",
                "content": ROUND_SYSTEM.format(agent="Brutus", round_num=2, prev_round=1),
            },
            context_msg,
            {
                "role": "user",
    ...
    publius_r3_raw = llm_call(
        [
            {
                "role": "system",
                "content": ROUND_SYSTEM.format(agent="Publius", round_num=3, prev_round=2),
            },
            context_msg,
            {
                "role": "user",
    ...
    brutus_r4_raw = llm_call(
        [
            {
                "role": "system",
                "content": ROUND_SYSTEM.format(agent="Brutus", round_num=4, prev_round=3),
            },
            context_msg,
            {
                "role": "user",
    ...
    record = {
        ...
        "process_metadata": {
            ...
            "termination": "round_limit",  # all run to 4 rounds in this batch
        },
...
def main():
    ...
    for tension in TENSIONS:
        try:
            ...
            # Write YAML record
            out_path = PROFILES_DIR / f"{record['id']}-v2.yaml"
