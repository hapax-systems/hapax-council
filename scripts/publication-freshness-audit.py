...
def _read_github_report(path: Path) -> GitHubPublicSurfaceReport:
    try:
        ...
        raise SystemExit(
            f"cannot read GitHub public-surface report: {path}: {exc}. "
            "Next action: provide a readable --github-report generated by "
            "scripts/github-public-surface-reconcile.py and hold release until "
            "publication-freshness-audit passes."
        ) from exc
    except json.JSONDecodeError as exc:
        raise SystemExit(
            f"malformed GitHub public-surface report: {path}: {exc}. "
