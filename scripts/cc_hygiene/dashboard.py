...
* ``## Research To Implementation`` — active queue rows whose task title,
  tags, parent plan, or parent spec indicate research-backed implementation.
...
def _vault_task_notes(active_root: Path) -> list[Any]:
    """Return parsed cc-task notes under ``active/`` for dashboard projections."""
    if not active_root.is_dir():
        return []
    notes: list[Any] = []
...
def _cell(value: Any, *, max_len: int = 56) -> str:
    """Sanitize one markdown-table cell."""
...
def _source_label(note: Any) -> str:
    raw = note.parent_spec or note.parent_plan
    if not raw:
        return "-"
    source = str(raw)
    if "/" in source:
        source = Path(source).name or source
    return _cell(source, max_len=48)


def _is_research_backed(note: Any) -> bool:
    haystack = " ".join(
        str(value)
        for value in (
            note.task_id,
...
def _priority_rank(priority: str | None) -> int:
    return {"p0": 0, "p1": 1, "p2": 2, "p3": 3}.get(str(priority or "").lower(), 9)
...
def _render_research_pipeline(active_root: Path) -> str:
    """Render research-backed active tasks so specs do not disappear after triage."""
    notes = [note for note in _vault_task_notes(active_root) if _is_research_backed(note)]
    header = "## Research To Implementation\n\n"
    if not notes:
        return header + "_No research-backed active tasks found._\n"

    counts = Counter(note.status for note in notes)
    counts_text = ", ".join(f"{status}: {count}" for status, count in sorted(counts.items()))
    rows: list[str] = [
        f"_Research-backed active queue: {counts_text}._",
        "",
        "| Task | Status | Role | P | WSJF | Source |",
        "|------|--------|------|---|------|--------|",
    ]
    ranked = sorted(
        notes,
        key=lambda note: (
            _status_rank(note.status),
            _priority_rank(note.priority),
            -(note.wsjf or 0.0),
            note.task_id,
        ),
    )
    for note in ranked[:RESEARCH_PIPELINE_LIMIT]:
        wsjf = "-" if note.wsjf is None else f"{note.wsjf:g}"
        rows.append(
            "| "
            f"{_cell(note.task_id)} | "
            f"{_cell(note.status)} | "
            f"{_cell(note.assigned_to)} | "
            f"{_cell(note.priority)} | "
            f"{_cell(wsjf)} | "
            f"{_source_label(note)} |"
        )
    if len(ranked) > RESEARCH_PIPELINE_LIMIT:
        rows.append(
            f"\n_Showing {RESEARCH_PIPELINE_LIMIT} of {len(ranked)} research-backed active tasks._"
        )
    return header + "\n".join(rows) + "\n"
...
def render_block(
    ...
    *,
    ...
    current_events = [event.model_dump(mode="json") for event in state.events]
    ...
    parts: list[str] = [
        ...
        "",
        _render_live_sessions(state, current_events),
        _render_research_pipeline(vault_active),
...
def render_unavailable_block(*, reason: str, now: datetime | None = None) -> str:
    """Build a sentinel block that makes missing hygiene state visible.

    This is used by the standalone dashboard renderer when the canonical
    ``cc-hygiene-state.json`` substrate is absent or invalid. The dashboard
    should show that the machine-readable state is missing instead of leaving a
    stale generated block that appears current.
    """
    now = now or datetime.now(UTC)
    ...
    parts: list[str] = [
        SENTINEL_START,
        "",
        f"_Generated by `cc-hygiene-dashboard-renderer` at {_fmt_ts(now)}._",
        "",
        "## Hygiene State Unavailable",
        "",
        f"**Status:** {safe_reason}",
        "",
        "The generated cc-hygiene dashboard cannot be trusted until "
        "`~/.cache/hapax/cc-hygiene-state.json` is regenerated by "
        "`scripts/cc-hygiene-sweeper.py`.",
        "",
        SENTINEL_END,
    ]
    return "\n".join(parts) + "\n"
...
def update_dashboard_unavailable(
    *,
    dashboard_path: Path = DEFAULT_DASHBOARD_PATH,
    reason: str,
    now: datetime | None = None,
) -> Path:
    """Rewrite the generated block with a visible unavailable-state notice."""
    if os.environ.get(KILLSWITCH_ENV) == "1":
        LOG.info("dashboard: killswitch active, skipping unavailable-state render")
        return dashboard_path

    block = render_unavailable_block(reason=reason, now=now)
    dashboard_path.parent.mkdir(parents=True, exist_ok=True)
    if not dashboard_path.exists():
        new_content = block
    else:
