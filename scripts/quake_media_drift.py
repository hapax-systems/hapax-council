...
def apply_frame_drift(
    ...
    *,
    ...
    mutation_pressure = _clamp01(
        0.35 + state.kind_variance * 0.45 + state.active_effect_ratio * 0.20
    )
    cadence_gain = _clamp01(
        0.62 + state.fast_ratio * fast_wave * 0.42 + state.slow_ratio * slow_wave * 0.26
    )
    ...
    for _ in range(min(18, max(0, block_count))):
        ...
        mix = min(
            0.22 if camera_receiver else 0.58,
            0.10 + intensity * 0.18 + state.texture * 0.12 + state.fast_ratio * 0.08,
        )
