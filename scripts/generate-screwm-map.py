...
SURFACE_CONTRACTS_BY_ROLE = {
    surface["role"]: surface for surface in SURFACE_CONTRACTS["surfaces"]
}
...
def validate_spatiotemporal_framework():
    ...
    for mount in MEDIA_MOUNT_CONTRACTS["mounts"]:
        ...
        if not isinstance(native, list) or len(native) != 2 or min(native) <= 0:
            failures.append(f"media mount {mount.get('id', '<unknown>')} has invalid native_resolution")
        ...
        if str(mount.get("projection", "")).startswith("flat"):
            ...
            if mount.get("visible_border") is not False:
                failures.append(f"flat media mount {mount.get('id', '<unknown>')} allows a visible border")
    ...
    if camera.get("review_path_period_s_target", 0) < camera.get(
        "review_path_period_s_min", 0
    ):
...
def validate_surface_contracts():
    ...
    if missing_info_fields:
        failures.append(f"information surface contract missing fields: {sorted(missing_info_fields)}")
    ...
    if "clean_room_homage_chrome" not in set(info_contract.get("admissible_texture_types", [])):
        failures.append("information surface contract must preserve clean-room Homage texture grammar")
    ...
    for surface in SURFACE_CONTRACTS["surfaces"]:
        ...
        if texture in forbidden_textures:
            failures.append(f"{surface.get('id', '<unknown>')} uses forbidden scenic texture {texture}")
...
def media_pane_brush(
    ...
    for face, p1, p2, p3 in plane_defs:
        if face == front_face or face == opposite_face:
            ...
            planes.append(fmt_plane(p1, p2, p3, live_tex, texture_scale, texture_rotation, texture_shift))
    return "{\n" + "\n".join(planes) + "\n}"
...
def generate_map(preset):
    ...
    lines.append('"_minlight" "6"')
    lines.append('"_minlight_color" "0.07 0.08 0.11"')
