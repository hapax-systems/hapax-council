...
def _paint_reverie_state_proxy(
    ...
    *,
    ...
    hue = (hue + _nested_float(visual_chain, "params.color.hue_rotate", "color.hue_rotate") * 160.0) % 360.0
    ...
    grad.add_color_stop_rgba(0.00, 0.02 + warmth * 0.10, 0.02, 0.12 + anonymize * 0.30, 0.78 * opacity)
    grad.add_color_stop_rgba(0.36, 0.36 + warmth * 0.42, 0.06 + pulse * 0.26, 0.36 + spectral * 0.40, 0.82 * opacity)
    ...
    for i in range(14):
        ...
        aura.add_color_stop_rgba(0, 1.0, 0.32 + warmth * 0.40, 0.78 + spectral * 0.22, 0.24 + pulse * 0.24)
    ...
    for i in range(10):
        ...
        cr.line_to(x0 + w * (0.55 + 0.20 * math.sin(phase)), y + h * (0.96 + 0.04 * math.cos(phase)))
        cr.stroke()
...
def _stale_source_reason(backend: Any, *, now: float, max_age_s: float) -> str | None:
    ...
    sidecar_path = Path(getattr(backend, "_sidecar_path", payload_path.with_suffix(payload_path.suffix + ".json")))
    try:
...
def main() -> int:
    ...
    parser.add_argument(
        "--drift-game-data",
        type=Path,
        default=Path(os.environ.get("HAPAX_QUAKE_WARD_ATLAS_DRIFT_GAME_DATA", str(DEFAULT_GAME_DATA))),
